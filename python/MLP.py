...
data2 = pd.read_csv('./data/MLP choose features 1.CSV', header=0, index_col=0)
data3 = pd.read_csv('./data/MLP choose features 2.CSV', header=0, index_col=0)
hidden1 = np.array(data2)
hidden2 = np.array(data3)
...
for i in range(20):
    ...
    for k, (train, test) in enumerate(skf):
        mlp = MLPClassifier(hidden_layer_sizes=(int(hidden1[i, k]), int(hidden2[i, k])),
                            solver='adam', random_state=1, max_iter=100000).fit(x[train], y[train])
        y_pre1 = mlp.predict(x[test])
